import { PRIVATE_KEY, ETH_NETWORK, ETH_CHAIN_ID, ETH_RPC_URL, ETH_RPC_WSS, SECONDS_BETWEEN_CHECK, ETH_LENDING_POOLS, ETH_FLASH_CONTRACT, ETH_FLASHLOAN_TOKEN, ETH_FLASHLOAN_TOKEN_DECIMALS } from "./config"
import { getAccountGasBalance, sendTelegramNotif, wait, format } from "./utils"
import { getPrice, buildTX } from "./tx"
import { ethers } from "ethers"

import LP_ABI from './utils/abi/mainnet.json'
import FLASH_ABI from '../contracts/abi/flash_liquidate_lamalend.json'

//const provider = new ethers.WebSocketProvider(ETH_RPC_WSS)
const provider = new ethers.JsonRpcProvider(ETH_RPC_URL)
const wallet = new ethers.Wallet(PRIVATE_KEY, provider)
const contract = new ethers.Contract(ETH_FLASH_CONTRACT, FLASH_ABI, wallet)
...
async function checkIfProfitable(liq: any, pool_address: string, collateral_token: string, collateral_token_decimals: number, collateral_token_symbol: string, borrowed_token: string, borrowed_token_decimals: number, borrowed_token_symbol: string) {
    ...
    const return_price = await getPrice(ETH_CHAIN_ID, ETH_FLASH_CONTRACT, collateral_token, collateral_amount.toString(), borrowed_token)
    ...
    //@ts-ignore
    if (profits_bi > 0n) {
        await sendTelegramNotif(`*${ETH_NETWORK.toUpperCase()}* | LIQ PROFITABLE FOUND`)
        const flashloan_token = ETH_FLASHLOAN_TOKEN
        const flashloan_token_decimals = ETH_FLASHLOAN_TOKEN_DECIMALS
        ...
        const tx_0_gen = await getPrice(ETH_CHAIN_ID, ETH_FLASH_CONTRACT, flashloan_token, flashloan_amount.toString(), borrowed_token)
        const tx_1_gen = await getPrice(ETH_CHAIN_ID, ETH_FLASH_CONTRACT, collateral_token, collateral_amount.toString(), flashloan_token)
        console.log(`> ⚡️ | \u001b[1;32mBUILDING TX\u001b[0m...`)
        const tx_0 = await buildTX(ETH_FLASH_CONTRACT, tx_0_gen.pathId, false)
        const tx_1 = await buildTX(ETH_FLASH_CONTRACT, tx_1_gen.pathId, false)
        const gas_limit = await contract.executeFlashLiquidation.estimateGas(0, flashloan_token, flashloan_amount.toString(), tx_0.transaction.data, user, pool_address, borrowed_token, collateral_token, tx_1.transaction.data)
        //@ts-ignore
        const up_gas_limit = gas_limit * 125n / 100n // + 25%
        const tx = await contract.executeFlashLiquidation(
            0, flashloan_token, flashloan_amount.toString(), tx_0.transaction.data, user, pool_address, borrowed_token, collateral_token, tx_1.transaction.data,
            { gasLimit: up_gas_limit }
        )
        console.log("> ⚡️ | TX sent =>", tx.hash)
        const receipt = await tx.wait()
        console.log(`> ⚡️ | \u001b[1;32mSUCCESS\u001b[0m => Tx confirmed ✔ ${receipt.blockNumber}`)
        await sendTelegramNotif(`*${ETH_NETWORK.toUpperCase()}* | LIQ EXECUTED`)
    } else return
}
...
async function iterate() {
    //let idx = 0
    while (true) {
        for (const [key, pool] of Object.entries(ETH_LENDING_POOLS)) {
...
async function run() {
    try {
        await getAccountGasBalance(ETH_NETWORK, ETH_RPC_URL, 6)
        await iterate()
    } catch (e) {
        console.error(e)
        await sendTelegramNotif(`LIQ - LMLend | ${ETH_CHAIN_ID} | got error at root | ${e}`)
